...
const float SCALE = 0.2;                // 1 => screen = 2x2m   2 => screen = 4x4m
const float WAVE_LENGTH = WAVE_SIM_SPEED / FREQ;
const float AMPLITUDE = 0.01;
const int NUM_OF_TRANSDUCERS = 1;
...
float wave_height(vec2 pos, float angle, float time) {
  float height = 0.0;
  for (int i = 0; i < NUM_OF_TRANSDUCERS; i++) {
    transducer t = get_transducer(i, angle);
    float y_view = 0.0;
    float dis = length(vec3(pos, y_view) - t.pos);
    float wave = AMPLITUDE / pow(dis, 2) * sin(2.0*PI*(FREQ*time - dis/WAVE_LENGTH) + t.phase);
    height += wave;
  }
  return height;
}
...
float radiation_pattern(vec2 pos, float angle) {
  float height = 0.0;
  for (int i = 0; i < 5; i++) {
    height = max(height, abs(wave_height(pos, angle, seconds*SIM_SPEED + float(i)*SIM_SPEED)));
  }
  return height;
}
...
void main() {
  ...
  if (uv.x > 0) {
    ...
    // float height = wave_height(uv, angle, seconds*SIM_SPEED);
    // vec3 color = color_map(height);
    float val = radiation_pattern(uv, angle);
    vec3 color;
    if (val > AMPLITUDE) {
      color = vec3(1, 1, 1);
    }
    // vec3 color = color_map(val);
